using Cbs.Data.Enums;
using Cbs.Data.Models.Application;
using Cbs.Data.Models.RequestModels;
using Cbs.Data.Models.ResponceModels;
using Cbs.Data.Models.ResponceModels.Other;
using Cbs.Web.UI.Dependency;

using Microsoft.AspNetCore.Http;

using RestSharp;

using System;
using System.Collections.Generic;
using System.Linq;
using System.Security.Claims;
using VYS.CacheManager.Core;

namespace Cbs.Web.UI.Helpers
{
    public class UserHelper
    {
        public static LoginResponceModel Login(string username, string password, string clientInfo, int timeOut)
        {
            LoginResponceModel responceModel = new LoginResponceModel();
            string responseContent = "";
            try
            {
                LoginRequestModel loginRequest = new LoginRequestModel();
                loginRequest.Username = username;
                loginRequest.Password = password;
                loginRequest.UserTimeOut = timeOut;
                loginRequest.ClientInfo = clientInfo;
                loginRequest.ApplicationTypeCode = (int)AppTypeEnum.Web;

                var client = RestHelper.GetClient("/api/User/Login");
                var request = RestHelper.GetRequest(Method.POST, Newtonsoft.Json.JsonConvert.SerializeObject(loginRequest));

                IRestResponse response = client.Execute(request);

                responseContent = response.Content;

                if (response.StatusCode == System.Net.HttpStatusCode.OK)
                {
                    responceModel = Newtonsoft.Json.JsonConvert.DeserializeObject<LoginResponceModel>(response.Content);
                }
                else
                {
                    responceModel.IsException = true;
                    responceModel.ExceptionMessage = response.ErrorMessage;
                }
            }
            catch (Exception ex)
            {
                responceModel.IsException = true;
                responceModel.ExceptionMessage = ex.ToString() + " Response Content: " + responseContent;
            }
            return responceModel;
        }

        public static void LogOut()
        {
            var client = RestHelper.GetClient("/api/User/LogOff");
            var request = RestHelper.GetRequest(Method.POST, null);
            var response = client.Execute(request);
        }

        public static SessionUserModel GetUser()
        {
            try
            {
                ICacheManager cacheManager = DependencyModule.Resolve<ICacheManager>();

                IHttpContextAccessor accessor = DependencyModule.Resolve<IHttpContextAccessor>();

                //var sessionId = accessor.HttpContext.Session.GetString("LogonUser");

                var sessionId = accessor.HttpContext.User.FindFirst(claim => claim.Type == System.Security.Claims.ClaimTypes.SerialNumber)?.Value;
                //var principal = new ClaimsPrincipal(sessionId.FirstOrDefault().);


                if (cacheManager.IsSet(sessionId))
                {
                    return cacheManager.Get<List<SessionUserModel>>(sessionId).FirstOrDefault();
                }
                else
                {
                    return new SessionUserModel();
                }
            }
            catch (Exception ex)
            {
                return new SessionUserModel();
            }
        }
        public static List<WebMenuResponseModelItem> GetUserMenus()
        {
            var user = GetUser();
            if (user != null && user.Id > 0)
            {
                return user.WebMenuItems;
            }
            else
            {
                return new List<WebMenuResponseModelItem>();
            }
        }

        public static string GetUserAvatar(long userId)
        {
            string responceModel = "";
            string responseContent = "";
            try
            {

                var client = RestHelper.GetClient("/api/User/GetUserAvatar");
                var request = RestHelper.GetRequest(Method.POST, Newtonsoft.Json.JsonConvert.SerializeObject(userId));

                IRestResponse response = client.Execute(request);

                responseContent = response.Content;

                if (response.StatusCode == System.Net.HttpStatusCode.OK)
                {
                    responceModel = Newtonsoft.Json.JsonConvert.DeserializeObject<string>(response.Content);
                }
                else
                {
                    responceModel = DefaultUserAvatar;
                }
            }
            catch (Exception ex)
            {
                responceModel = DefaultUserAvatar;
            }
            return responceModel;
        }

        public static string DefaultUserAvatar
        {
            get
            {
                return "data:image/png;base64,";
            }
        }

    }
}
